...
測量座標表の**写真 / PDF** から X・Y 座標を Claude(ビジョン)で読み取り、**行ごとの信頼度**を見ながら人間が確認・修正し、確定したテキスト(各行 `X Y`)を**ダウンロード**するWebアプリです。JW_cad に読み込んで作図するのが最終目的です。

## 特長
- 入力は写真(JPG/PNG)と PDF の両対応
- Claude が各行の **信頼度(%)** を返し、しきい値未満の行を自動でハイライト → チェックの力の入れどころが分かる
- 出力は 1 ファイル、各行 `X Y`(半角スペース区切り、写真通り 左X・右Y)
- txt はブラウザ内で生成してそのままダウンロード(メール・外部サービス不要)
- Claude APIキーは Supabase の Secret に保管(クライアントに出さない)
...
本アプリは **ダウンロード方式** です。txt はブラウザ内で生成されるため、メール送信の設定は不要で、登録するキーは1つだけです。

Supabase ダッシュボード → プロジェクト `zahyo-text` → **Edge Functions → Secrets** で、次を登録:
...
> `ANTHROPIC_API_KEY` を登録すれば、アップロード→確認→ダウンロードがすぐ使えます。
...
3. txt プレビューを確認 →「txt をダウンロード」
4. ダウンロードした txt を元データと見比べて最終確認
